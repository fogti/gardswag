...
use gardswag_syntax::{Interner, Expr, Symbol};
...
mod ffi {
    unsafe extern "C++" {
        ...
        fn emitFixpoint(&mut self, innerV: const Value *, namelen: usize, namedat: const u8 *) -> const Value *;
    }
}
...
fn codegen_expr(ctx: &mut Context, itn: &Interner, vs: &VarStack<'_, Symbol, const ffi::Value *>, expr: &Expr<InferExtra>) -> const ffi::Value* {
    ...
    match expr {
        ...
        Ek::Identifier(i) => *vs.find(i).expect("unknown identifier encountered during codegen"),
        Ek::PureString(s) => ctx.getString(s.len(), s.bytes()),

        // code structure
        Ek::Let { lhs, rhs, rest } => {
            let v_rhs = codegen_expr(ctx, itn, vs, rhs);
            codegen_block(ctx, itn, VarStack {
                psrent: vs,
                name: lhs,
                value: v_rhs,
            }, rest)
...
fn codegen_block(ctx: &mut Context, itn: &Interner, vs: &VarStack<'_, Symbol, const ffi::Value *>, blk: &Block<InferExtra>) -> const ffi::Value* {
    for i in &blk.stmts {
